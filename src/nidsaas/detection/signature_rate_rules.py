...
-----
    ...
    PYTHONPATH=src python -m nidsaas.detection.signature_rate_rules \
        --data-dir data/csv/csv_CIC_IDS2017 \
        --output-csv data/samples/signature_rate_predictions.csv

    # OR-merge with the existing Snort predictions into a single CSV:
    PYTHONPATH=src python -m nidsaas.detection.signature_rate_rules \
        --data-dir data/csv/csv_CIC_IDS2017 \
        --merge-snort-csv outputs/snort_eval_v4a/snort_signature_predictions.csv \
        --output-csv data/samples/signature_merged_predictions.csv
...
Then feed the output straight into the cascade:
    ...
    python scripts/run_pipeline.py \
        ...
        --alpha-escalate 0.20 \
        --output-dir outputs/hybrid_cascade_splitcal_dualfast_temporal
"""
...
from nidsaas.detection.load_data import clean_detection_dataframe, read_cic_ids2017_folder
